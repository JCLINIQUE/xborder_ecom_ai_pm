...
| 阶段 | 主要内容 |
| --- | --- |
| 第一周 · Day 01–07 | Day 01–03：调研 → 候选需求 → 功能推导与业务初稿；Day 04：竞品追踪工作台 PRD；随后完成对应原型与 Demo |
...
Day 01–04 聚焦**日常竞品跟踪如何辅助运营决策**。Day 01 先区分产品开发／选品的市场调研与运营的日常跟踪，再示范访谈和报告；Day 02 从核验、比较与判断中提出 2–3 条候选需求，标明证据缺口并推荐一项；Day 03 先浏览完整 PRD 框架，再从一条价格／促销跟踪需求推导功能，参照同一方案的示例写成业务版 PRD 初稿。Day 04 根据明确后的首版范围，将数据整理与对照展开为工作台 PRD：截图／Excel／Word 导入 → 提取与核对 → 跟踪表与比较 → 导出，确定不做爬虫，经营建议后置。

现有材料提供了跟踪流程和判断思路，完整的连续记录与真实处理案例仍待补充。课堂模拟样例单独标注，不作为已验证的需求或收益；桌麦市场报告归入市场调研参考，不与跟踪证据混用。

业务版是同一份 PRD 的第一版，先写清业务行为与验收，再随学习补充 AI 交互、架构、数据和评测。Day 04 先完成工作台的业务规则；后续先完成对应原型，再制作 Demo。目录内旧 SSD 原型不是本稿的配套材料。
...
| `attachments/4. prototype/` | **Day 04 主材料：[竞品追踪工作台 · 业务版 PRD](<attachments/4. prototype/业务版PRD-竞品追踪工作台.md>)**。承接 Day 03，写清多格式导入、提取核对、按时点积累、对照看数与导出。目录名沿用，旧 SSD 原型仅作历史参考。 |
