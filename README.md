...
| 阶段 | 主要内容 |
| --- | --- |
| 第一周 · Day 01–07 | Day 01–03：调研 → 候选需求 → 功能推导与业务初稿；Day 04：竞品追踪工作台 PRD 与三条原型路径；随后制作 Demo |
...
业务版是同一份 PRD 的第一版，先写清业务行为与验收，再随学习补充 AI 交互、架构、数据和评测。Day 04 用“资料准备 → 分析工作台”的可点击原型走查正常完成、部分失败、修改后三条路径，随后制作 Demo。原型中的提取和模型结果均为教学模拟；目录内旧 SSD 原型仅作历史参考。
...
| `attachments/4. prototype/` | [业务版 PRD](<attachments/4. prototype/业务版PRD-竞品追踪工作台.md>) → [可点击原型：三条路径](<attachments/4. prototype/竞品追踪工作台-交互原型.html>) → [课堂走查指南](<attachments/4. prototype/Day04原型走查指南.md>)。从数据整理、程序对比到模型辅助解读，练习正常完成、部分失败与修改后的复核。 |
