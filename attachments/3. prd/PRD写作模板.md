...
当前 Day 02–03 按“调研证据 → 候选需求 → 产品功能与首版范围”推进。Day 02 提出 2–3 条候选需求、标明证据缺口并推荐一项；Day 03 阅读 [产品功能设计](<Day03产品功能设计.md>)，完成 [课堂练习与功能清单模板](<Day03课堂练习与功能清单模板.md>)，交付需求—功能对应表、首版功能清单及输入输出样例。

课堂主线是“日常竞品跟踪 → 核验记录与识别价格／促销变化 → 辅助排查 → 运营决定”。产品复用现有工具结果，区分事实、可能解释和待核查项，由运营决定是否跟进；课堂模拟不作真实业务证据。需求与首版功能明确后，再把它们写成 PRD；不预设必须沿用旧版 R1–R4 或“录入—比较—导出”方案。[旧版 SSD PRD](<业务版PRD-竞品对比助手.md>) 仅用于参考规则和验收的写法，不是当前 Day 03 的产出。

**Day 04–30 及后续阶段目标保留旧安排，待按新功能设计衔接。** 旧安排中“承接 Day 03 的 PRD”等表述不构成当前的交付依赖。以下是后续进入 PRD 时的选读范围，不是 Day 03 当天的全量作业；完整模板与写作原则保留作进阶参考。

| 初稿内容 | 参考章节 | 进入 PRD 后写到什么程度 |
| --- | --- | --- |
| 背景与用户问题 | 1、4.1–4.4 | 目标用户、现有做法、具体困难与调研证据；对应 Day 02 推荐项，并标明尚待验证的判断。 |
| 定位与首版范围 | 2.1、2.4 | 承接 Day 03 的需求—功能对应表，说明核心任务、首版功能与取舍依据。 |
| 业务目标 | 3.1 | 如何判断任务完成、如何观察耗时或错误变化；尚未测量的基线写“待测量”，注明采集方法。 |
| 用户流程与页面草图 | 4.5、6.2 | 主要操作步骤、预计需要的页面和页面目标；后续完成原型后补充链接与交互说明。 |
| 功能与规则 | 7.1.1、7.1.3–7.1.5、7.1.9、7.1.11 | 每项核心功能的触发、输入、处理规则、输出，以及缺少数据、格式错误等异常；按需要明确人工复核点，用普通流程图或文字描述即可。 |
| 功能验收 | 14.2 | 将 Day 03 的输入输出样例展开为“给定输入 → 用户操作 → 预期结果”，补充正常与异常情况，供后续检查实现。 |
| 待确认事项 | 16 | 未确定的问题、影响和下一步如何确认。 |

Day 03 不要求填写完整模板。进入 PRD 后，AI 交互、架构与路由、模型选型、质量评测等内容应随相应教学逐步补齐；尚未理解或确定的部分登记为“待学习后补 / 待审查”，不凭空补写。项目不采用的能力标为“不涉及”并说明原因。需要引用功能时保留简单、稳定的编号即可。

后续制作 Demo 时，未实现、模拟数据或模拟生成结果须明确标注。初始实现能运行不代表模型调用、交互和架构已经合理；需结合后续课程审查，并同步修订 PRD、原型、实现与验收。各章中的完整交付要求及第 18 节自检按项目适用范围使用。
