...
>
> 使用方式：完整项目复制“模板正文”，替换 `{{...}}`；标有“按需”的章节不涉及时写明原因。第一周课程按下方说明选取内容。PRD 要回答四件事：**为什么做、做什么、系统如何表现、如何证明做成了。**

## 第一周课程说明

Day 03 先看 [业务版 PRD 编写指南](<Day03业务版PRD编写指南.md>)，再对照 [竞品对比助手业务版 PRD](<业务版PRD-竞品对比助手.md>) 完成自己的版本。示例中的业务规则属于教学验证草案，需用真实任务确认。

第一周按“调研 → 产品需求提炼 → 产品 PRD → 产品原型图 → 产品 Demo”推进。Day 02 先形成带调研证据、优先级和首版范围的需求清单；Day 03 再写业务版 PRD，供 Day 04 画原型图、Day 05 做 Demo。完整模板作为进阶参考，第一周只按下表选取相关内容，建议控制在 2–3 页。

第一周可用 vibe coding 制作 Demo，首版即可包含 AI 功能。初始实现仍待审查：能运行不代表学员已理解其中的模型调用、交互与技术架构。第二周沿用同一份 PRD 和同一个 Demo，逐步解释、核对并修订已有实现。

| 必填内容 | 参考章节 | 第一周写到什么程度 |
| --- | --- | --- |
| 背景与用户问题 | 1、4.1–4.4 | 目标用户、现有做法、具体困难与调研证据；需求能对应 Day 02 的清单。 |
| 定位与首版范围 | 2.1、2.4 | 解决什么问题、本期做什么和不做什么；说明优先级依据。 |
| 业务目标 | 3.1 | 如何判断任务完成、如何观察耗时或错误变化；尚未测量的基线写“待测量”，注明采集方法。 |
| 用户流程与页面草图 | 4.5、6.2 | 主要操作步骤、预计需要的页面和页面目标；Day 04 完成原型图后补充链接与交互说明。 |
| 功能与规则 | 7.1.1、7.1.3–7.1.5、7.1.9、7.1.11 | 每项核心功能的触发、输入、处理规则、输出，以及缺少数据、格式错误等异常；按需要明确人工复核点，用普通流程图或文字描述即可。 |
| 功能验收 | 14.2 | 用“给定输入 → 用户操作 → 预期结果”写正常与异常样例，供 Day 05–07 检查 Demo。 |
| 待确认事项 | 16 | 未确定的问题、影响和下一步如何确认。 |

第一周不要求填写 Agent 架构与路由、Skill 生命周期、模型选型、AI 专项指标和评测、完整埋点与灰度治理，也不要求建立完整版的全部编号与追踪矩阵。需要引用功能时保留简单、稳定的需求编号即可。Demo 中未实现或使用模拟数据的部分须明确标注。

尚未理解或确定的 AI 方案登记为“待学习后补 / 待审查”；项目不采用的能力标为“不涉及”并说明原因。Day 09–11 学习 AI 概念、交互范式与现有技术架构后，逐日补充产品机制、交互规则和技术取舍；Day 12 对照 PRD、原型与 Demo 评审一致性，并在课后完成文档补充；Day 13 完善手机学习 PWA，Day 14 定向修订主项目。Day 15–18 学习离线评测并完成对照改进后，更新质量要求、验收与版本记录；Day 19 完善发布协作要求，Day 22–23 补充适用的实验设计与结果判断。各章中的完整交付要求及第 18 节自检按项目适用范围使用，不作为第一周的全量作业标准。
