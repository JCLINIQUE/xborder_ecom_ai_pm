...
在「数据导入 → MCP 数据源」粘贴西柚洞察控制台「凭证管理」提供的**完整 MCP 授权链接**，点击连接，选择查询类型并填写 ASIN / 关键词 / 站点 / 时间范围，然后勾选额度确认并读取预览。点击「导入这份结果」后才会保存；仍需在原校验页确认，才会参与报告。

- Codex 中的连接与独立工作台是两个客户端，不会自动共享。工作台不读取 Codex 凭证文件；链接只保留在弹窗内存中，关闭或刷新即清空，不保存到数据库、日志、Prompt、备份或日报。
